...
internal sealed class TravelContextResolver : ITravelContextResolver
{
    ...
    public async Task<TravelResolution> ResolveAsync(TravelRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.IsBack)
        {
            ...
            {
                var areas = DCTravelClient.Areas.OrderBy(x => x.Key).Select(x => x.Value.Area);

                var result = await WindowManager.WorldSelector!.OpenTravelWindow(
                    true, false, true,
                    areas,
                ...
                if (result?.Source is not { } selectedSourceGroup)
                    return new(sourceContext, request.Title, targetDcGroupName, null, null, retrySettings, string.Empty, false, false);

                sourceContext = sourceContext with { CurrentGroup = selectedSourceGroup };
            }
        ...
        else
        {
            ...
            var result = await WindowManager.WorldSelector!.OpenTravelWindow(
                false, true, false,
                areas,
                sourceGroup: sourceContext.CurrentGroup,
                targetGroup: null
            );

            if (result?.Target is not { } selectedTargetGroup)
            {
            ...
            requestedTargetGroup = selectedTargetGroup;
        }
